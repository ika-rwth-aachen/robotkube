...
find $SCRIPT_DIR/kubernetes/initial_deployment/cloud -maxdepth 1 -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: v1/ && /kind: Service/' {} + | kubectl apply -f - 1>/dev/null 2>&1
find $SCRIPT_DIR/kubernetes/initial_deployment/vehicles -maxdepth 1 -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: v1/ && /kind: Service/' {} + | kubectl apply -f - 1>/dev/null 2>&1
find $SCRIPT_DIR/kubernetes/initial_deployment/cloud/mqtt -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: v1/ && /kind: Service/' {} + | kubectl apply -f - 1>/dev/null 2>&1
find $SCRIPT_DIR/kubernetes/initial_deployment/edge/mqtt -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: v1/ && /kind: Service/' {} + | kubectl apply -f - 1>/dev/null 2>&1
find $SCRIPT_DIR/kubernetes/initial_deployment/vehicles/mqtt -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: v1/ && /kind: Service/' {} + | kubectl apply -f - 1>/dev/null 2>&1
...
kubectl create configmap mqtt-params-cloud --from-file=$SCRIPT_DIR/kubernetes/ros_paramsfiles/mqtt/cloud/mqtt_params_cloud.yml \
                                           -o yaml --dry-run=client | sed 's/^metadata:/metadata:\n  labels:\n    tag: robotkube/' | kubectl apply -f - 1>/dev/null
kubectl create configmap mqtt-params-vehicles --from-file=$SCRIPT_DIR/kubernetes/ros_paramsfiles/mqtt/vehicles/mqtt_params_vehicle_00.yml \
                                              --from-file=$SCRIPT_DIR/kubernetes/ros_paramsfiles/mqtt/vehicles/mqtt_params_vehicle_01.yml \
                                              --from-file=$SCRIPT_DIR/kubernetes/ros_paramsfiles/mqtt/vehicles/mqtt_params_vehicle_02.yml \
                                              --from-file=$SCRIPT_DIR/kubernetes/ros_paramsfiles/mqtt/vehicles/mqtt_params_vehicle_03.yml \
                                              -o yaml --dry-run=client | sed 's/^metadata:/metadata:\n  labels:\n    tag: robotkube/'| kubectl apply -f - 1>/dev/null
kubectl create configmap event-detector-operator-params --from-file=$SCRIPT_DIR/kubernetes/ros_paramsfiles/event_detector_operator.params.yml -o yaml --dry-run=client | sed 's/^metadata:/metadata:\n  labels:\n    tag: robotkube/' | kubectl apply -f - 1>/dev/null
kubectl create configmap application-manager-params --from-file=$SCRIPT_DIR/kubernetes/ros_paramsfiles/application_manager.params.yml -o yaml --dry-run=client | sed 's/^metadata:/metadata:\n  labels:\n    tag: robotkube/' | kubectl apply -f - 1>/dev/null
kubectl create configmap application-registry-interface --from-file=$SCRIPT_DIR/kubernetes/application_registry_interface/application_registry_interface.yml -o yaml --dry-run=client | sed 's/^metadata:/metadata:\n  labels:\n    tag: robotkube/' | kubectl apply -f - 1>/dev/null
kubectl create configmap kubernetes-operator-configuration --from-file=$SCRIPT_DIR/kubernetes/operator_config/config.yml -o yaml --dry-run=client | sed 's/^metadata:/metadata:\n  labels:\n    tag: robotkube/' | kubectl apply -f - 1>/dev/null
kubectl create configmap mosquitto-broker-edge-configmap --from-file=$SCRIPT_DIR/kubernetes/mosquitto_config/edge/mosquitto.conf -o yaml --dry-run=client | sed 's/^metadata:/metadata:\n  labels:\n    tag: robotkube/' | kubectl apply -f - 1>/dev/null
...
kubectl apply -f $SCRIPT_DIR/kubernetes/initial_deployment/cloud/kopf_operator_mqtt.yml 1>/dev/null
kubectl apply -f $SCRIPT_DIR/kubernetes/initial_deployment/cloud/kopf_operator_object_detection.yml 1>/dev/null
kubectl apply -f $SCRIPT_DIR/kubernetes/initial_deployment/cloud/kopf_operator_object_fusion.yml 1>/dev/null
...
find $SCRIPT_DIR/kubernetes/initial_deployment/station -maxdepth 1 -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | kubectl apply -f - 1>/dev/null
find $SCRIPT_DIR/kubernetes/initial_deployment/vehicles -maxdepth 1 -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | kubectl apply -f - 1>/dev/null
find $SCRIPT_DIR/kubernetes/initial_deployment/cloud/mqtt -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | kubectl apply -f - 1>/dev/null
find $SCRIPT_DIR/kubernetes/initial_deployment/edge/mqtt -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | kubectl apply -f - 1>/dev/null
find $SCRIPT_DIR/kubernetes/initial_deployment/vehicles/mqtt -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | kubectl apply -f - 1>/dev/null
...
expected_pod_count=$(find $SCRIPT_DIR/kubernetes/initial_deployment/cloud -maxdepth 1 -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | grep -c '^---$')
expected_pod_count=$((expected_pod_count + $(find $SCRIPT_DIR/kubernetes/initial_deployment/station -maxdepth 1 -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | grep -c '^---$')))
expected_pod_count=$((expected_pod_count + $(find $SCRIPT_DIR/kubernetes/initial_deployment/vehicles -maxdepth 1 -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | grep -c '^---$')))
expected_pod_count=$((expected_pod_count + $(find $SCRIPT_DIR/kubernetes/initial_deployment/cloud/mqtt -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | grep -c '^---$')))
expected_pod_count=$((expected_pod_count + $(find $SCRIPT_DIR/kubernetes/initial_deployment/edge/mqtt -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | grep -c '^---$')))
expected_pod_count=$((expected_pod_count + $(find $SCRIPT_DIR/kubernetes/initial_deployment/vehicles/mqtt -type f -name "*.yml" -exec awk 'BEGIN{RS="\n\n";ORS="\n---\n"} /apiVersion: apps\/v1/ && /kind: Deployment/' {} + | grep -c '^---$')))
...
# Get the names of all pods containing "xx-publisher"
pod_names_rosbag=$(kubectl get pods | grep vehicle-.*-publisher | awk '{print $1}')
pod_names_rosbag+=" $(kubectl get pods | grep station-.*-publisher | awk '{print $1}')"

# All pods are running, print their names and the current date and do ros2 bag play
for pod_name in $pod_names_rosbag; do
    # Start the rosbag
    entity=$(echo $pod_name | awk -F '-' '{print $1}')
    id=$(echo $pod_name | awk -F '-' '{print $2}')
    kubectl exec $pod_name -c rosbag-play  -- bash -c "echo $pod_name $(date +'%T.%3N') ; . /opt/ros/\$ROS_DISTRO/setup.bash ; source install/setup.bash ; ros2 bag play -l -r 1 /data/${entity}${id} >/dev/null 2>&1 &  " >/dev/null &
done
